...
import { startOfWeek, endOfWeek, addMonths, subMonths, format, isSameMonth, parseISO } from 'date-fns';
import { LifestyleGoal, Wallet, Transaction, Category, LifestyleSettings, MotivationalEarning, EarningAction } from '../../types';
import { formatCurrency, getLifestyleColor } from '../../lib/utils';
import { PlusCircle, Trash2, Trophy, ShieldCheck, Heart, PiggyBank, Info, Plus, ChevronLeft, ChevronRight, Settings, ArrowDownCircle, ArrowUpCircle, Target, CheckCircle2, Circle, Upload, Edit2, Calendar, AlignLeft, Tag, Wallet as WalletIcon } from 'lucide-react';
import { Modal } from '../../components/Modal';
import { TransactionCard } from '../../components/TransactionCard';
import { MotivationalEarningSection } from './components/MotivationalEarningSection';
import { motion, AnimatePresence } from 'motion/react';
import { useSwipeable } from 'react-swipeable';
import { LifestyleViewProps } from './types';
...
export function LifestyleView({ 
  ...
  const [selectedCategory, setSelectedCategory] = useState<'need' | 'want' | 'savings' | null>(null);
  const [currentDate, setCurrentDate] = useState(new Date());
  const [currentSlideIndex, setCurrentSlideIndex] = useState(0);
  const [direction, setDirection] = useState(0);
  const [tLifestyleType, setTLifestyleType] = useState<'need' | 'want' | 'savings' | 'income'>('need');
  ...
  // Settings state
  const [incomeSource, setIncomeSource] = useState<'default_wallet' | 'custom'>(lifestyleSettings?.incomeSource || 'default_wallet');
  const [customIncomeAmount, setCustomIncomeAmount] = useState(lifestyleSettings?.customIncomeAmount?.toString() || '');
  const [percentages, setPercentages] = useState(lifestyleSettings?.percentages || { need: 50, want: 30, savings: 20 });

  // Update settings state when props change
  React.useEffect(() => {
    if (lifestyleSettings) {
      setIncomeSource(lifestyleSettings.incomeSource);
      setCustomIncomeAmount(lifestyleSettings.customIncomeAmount?.toString() || '');
      setPercentages(lifestyleSettings.percentages || { need: 50, want: 30, savings: 20 });
    }
  }, [lifestyleSettings]);

  const handleOpenLogTransaction = (type?: 'need' | 'want' | 'savings') => {
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    onUpdateLifestyleSettings({
      incomeSource,
      customIncomeAmount: incomeSource === 'custom' ? Number(customIncomeAmount) : undefined,
      percentages
    });
  ...
  const monthlyIncomeTransactions = transactions.filter(t => t.type === 'income' && isSameMonth(parseISO(t.date), currentDate));
  const actualMonthlyIncome = monthlyIncomeTransactions.reduce((acc, t) => acc + t.amount, 0);

  const totalMonthlyIncome = lifestyleSettings?.incomeSource === 'custom' 
  ...
  const totals = {
    need: goals.filter(g => g.category === 'need').reduce((acc, g) => acc + calculateMonthlyRequirement(g), 0),
    want: goals.filter(g => g.category === 'want').reduce((acc, g) => acc + calculateMonthlyRequirement(g), 0),
    savings: goals.filter(g => g.category === 'savings').reduce((acc, g) => acc + calculateMonthlyRequirement(g), 0),
  };

  const totalRequirement = totals.need + totals.want + totals.savings;

  const currentPercentages = lifestyleSettings?.percentages || { need: 50, want: 30, savings: 20 };
  ...
  const monthlyTransactions = transactions.filter(t => isSameMonth(parseISO(t.date), currentDate));
  ...
  const totalRemaining = Math.max(0, ruleTargets.need - actualSpending.need) + 
        ...
        {totalMonthlyIncome > 0 ? (
          <div className="bg-royal text-white p-6 rounded-3xl shadow-xl shadow-royal/20 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center mb-1">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-royal-light">Total Monthly Remaining</p>
              </div>
          ...
          <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-lg relative overflow-hidden">
            ...
            <p className="text-slate-400 text-sm leading-relaxed relative z-10">
              Apply your custom allocation rule to your dream lifestyle. Define what you need, what you want, and what you want to save.
            </p>
          </div>
        )}
        ...
        {selectedCategory ? (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className={`px-3 py-1 rounded-full ${getCategoryBg(selectedCategory)} ${getCategoryColor(selectedCategory)} text-[10px] font-bold uppercase tracking-widest`}>
                {selectedCategory}
              </div>
            </div>

            <div className="flex items-center space-x-4">
              <div className={`w-12 h-12 rounded-2xl ${getCategoryBg(selectedCategory)} ${getCategoryColor(selectedCategory)} flex items-center justify-center`}>
                {getCategoryIcon(selectedCategory)}
              </div>
              <div>
                <h2 className="text-2xl font-bold text-slate-800 capitalize">{selectedCategory}s</h2>
                <p className="text-xs text-slate-400 font-medium">
                  {selectedCategory === 'savings' ? 'Goal' : 'Monthly Limit'}: {formatCurrency(ruleTargets[selectedCategory], currency)}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                    {selectedCategory === 'savings' 
                      ? (ruleTargets[selectedCategory] - actualSpending[selectedCategory] < 0 ? 'Surplus' : 'Remaining Goal') 
                      : 'Remaining Budget'}
                  </p>
                  <p className={`text-3xl font-bold ${selectedCategory !== 'savings' && ruleTargets[selectedCategory] - actualSpending[selectedCategory] < 0 ? 'text-rose-500' : getCategoryColor(selectedCategory)}`}>
                    {selectedCategory === 'savings' && ruleTargets[selectedCategory] - actualSpending[selectedCategory] < 0
                      ? `+${formatCurrency(Math.abs(ruleTargets[selectedCategory] - actualSpending[selectedCategory]), currency)}`
                      : formatCurrency(ruleTargets[selectedCategory] - actualSpending[selectedCategory], currency)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                    {selectedCategory === 'savings' ? 'Saved This Month' : 'Spent This Month'}
                  </p>
                  <p className="text-lg font-bold text-slate-800">{formatCurrency(actualSpending[selectedCategory], currency)}</p>
                </div>
              </div>
              <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ 
                    width: `${Math.max(0, Math.min(100, selectedCategory === 'savings' 
                      ? (actualSpending[selectedCategory] / ruleTargets[selectedCategory]) * 100 
                      : 100 - (actualSpending[selectedCategory] / ruleTargets[selectedCategory]) * 100))}%` 
                  }}
                ></div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="pt-4 border-t border-slate-100">
                <h3 className="text-sm font-bold text-slate-800 px-1 mb-4">{selectedCategory === 'savings' ? 'Savings Goals' : 'Planned Monthly Expenses'}</h3>
                {goals.filter(g => g.category === selectedCategory).length === 0 && monthlyTransactions.filter(t => categories.find(c => c.name === t.category)?.lifestyleType === selectedCategory && !goals.filter(g => g.category === selectedCategory).map(g => g.title).includes(t.category)).length === 0 ? (
                  <div className="text-center py-12 bg-white rounded-3xl border-2 border-dashed border-slate-100">
                    <p className="text-slate-400 text-sm font-medium">No {selectedCategory} items defined yet.</p>
                    <button 
                  ...
                  <div className="space-y-4">
                    {goals.filter(g => g.category === selectedCategory).map(goal => {
                      const goalTransactions = transactions.filter(t => t.category === goal.title && t.type === (selectedCategory === 'income' ? 'income' : 'expense'));
                      const actualAmount = goalTransactions.reduce((acc, t) => acc + t.amount, 0);
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <h4 className="font-bold text-slate-800 pr-8">{goal.title}</h4>
                          </div>

                          <div className="flex justify-between items-baseline mb-3">
                            <p className={`font-mono font-bold ${getCategoryColor(selectedCategory)}`}>
                              {selectedCategory === 'savings' ? (
                                <>
                                  {formatCurrency(goal.targetAmount, currency)}
                                  <span className="text-[10px] text-slate-400 font-sans font-normal uppercase tracking-widest ml-2">Target</span>
                                </>
                              ) : (
                                <>
                                  {formatCurrency(goal.targetAmount, currency)} 
                            ...
                            <div className="mb-4">
                              <div className="flex justify-between items-end mb-1">
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                                  {actualAmount > goal.targetAmount ? 'Overspent' : 'Remaining'}
                                </p>
                                <p className={`text-xs font-bold ${actualAmount > goal.targetAmount ? 'text-rose-500' : `text-${getLifestyleColor(selectedCategory || undefined)}`}`}>
                                  {actualAmount > goal.targetAmount 
                                    ? `-${formatCurrency(actualAmount - goal.targetAmount, currency)}`
                                    : formatCurrency(goal.targetAmount - actualAmount, currency)}
                                </p>
                              </div>
                              <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div 
                                  ...
                                  style={{ width: `${Math.max(0, 100 - (actualAmount / goal.targetAmount) * 100)}%` }}
                                ></div>
                              </div>
                            </div>
                          )}
                          ...
                          {goalTransactions.length > 0 && (
                            <div className="space-y-2 mt-4 pt-4 border-t border-slate-50">
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-2">Transactions</p>
                              {goalTransactions.slice(0, 3).map(t => (
                                <div key={t.id} className="flex justify-between items-center text-xs">
                                  <span className="text-slate-600 truncate pr-4">{t.description || t.category}</span>
                                  <span className="font-mono font-medium text-slate-800">{formatCurrency(t.amount, currency)}</span>
                                </div>
                              ))}
                              {goalTransactions.length > 3 && (
                                <p className="text-[10px] text-slate-400 text-center pt-1">+{goalTransactions.length - 3} more</p>
                              )}
                            </div>
                          )}
                          ...
                          {selectedCategory === 'savings' && (
                            <div className="mb-2">
                              <div className="flex justify-between items-end mb-1">
                                ...
                                <p className={`text-xs font-bold text-notion-green`}>{Math.round(((goal.currentAmount || 0) / goal.targetAmount) * 100)}%</p>
                              </div>
                              <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div 
                              ...
                              <div className="flex justify-between mt-1">
                                <p className="text-[10px] text-slate-400">{formatCurrency(goal.currentAmount || 0, currency)} saved</p>
                              </div>
                              <div className={`mt-3 p-2 bg-notion-green-light rounded-lg border border-notion-green-light`}>
                                <p className={`text-[10px] text-notion-green font-medium`}>
                                  Monthly target: <span className="font-bold">{formatCurrency(calculateMonthlyRequirement(goal), currency)}</span>
                                </p>
                              </div>
                            </div>
                          )}
                    ...
                    {/* Unplanned Card */}
                    {(() => {
                      const goalTitles = goals.filter(g => g.category === selectedCategory).map(g => g.title);
                      const unplannedTransactions = monthlyTransactions
                        .filter(t => categories.find(c => c.name === t.category)?.lifestyleType === selectedCategory && !goalTitles.includes(t.category));
                      
                      if (unplannedTransactions.length === 0) return null;
                      
                      const totalOthers = unplannedTransactions.reduce((acc, t) => acc + t.amount, 0);
                      ...
                      return (
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 relative group hover:border-royal/20 transition-all">
                          <div className="flex justify-between items-center mb-4">
                            <div>
                              <h4 className="font-bold text-slate-800">Unplanned {selectedCategory}</h4>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Total Summary</p>
                            </div>
                            <div className="text-right">
                              <p className="text-lg font-mono font-bold text-slate-900">{formatCurrency(totalOthers, currency)}</p>
                            </div>
                          </div>
                          
                          <div className="space-y-2 mt-4 pt-4 border-t border-slate-50">
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-2">Recent Items</p>
                            ...
                            {unplannedTransactions.length > 5 && (
                              <p className="text-[10px] text-slate-400 text-center pt-1">+{unplannedTransactions.length - 5} more</p>
                            )}
                          </div>
                        </div>
                      );
                ...
                {currentSlide === 'motivational' ? (
                  ...
                  <div className="bg-amber-50 border border-amber-100 p-6 rounded-3xl flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-slate-500 mt-2 mb-6 max-w-xs">
                      To calculate your monthly targets, we need to know your monthly budget.
                    </p>
                    <button 
                      ...
                      { 
                        ...
                        >
                          ...
                          <p className="text-[10px] text-slate-400 font-medium mt-1">
                            {item.key === 'savings' 
                              ? (item.remaining < 0 ? 'Surplus' : 'Remaining goal') 
                              : 'Remaining limit'}
                          </p>
                          ...
                          <div className="mt-4 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                            <div 
                              ...
                              style={{ 
                                width: `${Math.max(0, Math.min(100, item.key === 'savings' 
                          ...
                          <div className="flex justify-between mt-2">
                            <p className="text-[10px] text-slate-400 font-medium">
                              {item.key === 'savings' ? 'Goal' : 'Limit'}: {formatCurrency(item.target, currency)}
                            </p>
                            <p className={`text-[10px] font-bold ${item.key !== 'savings' && item.current > item.target ? 'text-rose-500' : 'text-notion-green'}`}>
                              {item.key === 'savings' ? 'Saved' : 'Spent'}: {formatCurrency(item.current, currency)}
                            </p>
                          </div>
