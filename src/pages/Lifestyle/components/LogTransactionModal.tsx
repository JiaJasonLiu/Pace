...
import { Calendar, Tag, Wallet as WalletIcon, AlignLeft } from 'lucide-react';
import { LogTransactionModalProps } from '../types';
...
export function LogTransactionModal({ 
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const filteredCats = categories.filter(c => c.lifestyleType === type && c.type === 'expense');
  ...
  const handleLifestyleTypeChange = (type: 'need' | 'want' | 'savings' | 'income') => {
    setTLifestyleType(type);
    const filteredCats = categories.filter(c => c.lifestyleType === type && c.type === (type === 'income' ? 'income' : 'expense'));
    setTCategory(filteredCats[0]?.name || '');
  };

  const handleTransactionSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!tAmount || isNaN(Number(tAmount)) || !tCategory || !tWalletId) return;
  ...
  return (
    ...
    >
      <form id="log-transaction-form" onSubmit={handleTransactionSubmit} className="space-y-3">
        ...
        <p className="text-center text-xs text-slate-400 font-medium -mt-2 mb-4">Amount</p>
        ...
        <div className="flex flex-col">
          ...
          <label className="flex items-center gap-4 p-4 border-b border-slate-100 cursor-pointer hover:bg-slate-50 transition-colors">
            ...
            >
              ...
              {categories
                .filter(c => c.lifestyleType === tLifestyleType && c.type === (tLifestyleType === 'income' ? 'income' : 'expense'))
                .map(cat => (
