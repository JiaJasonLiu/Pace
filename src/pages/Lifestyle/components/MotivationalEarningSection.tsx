...
import { MotivationalEarning, EarningAction } from '../../../types';
import { formatCurrency } from '../../../lib/utils';
import { Target, Plus, CheckCircle2, Circle, Upload, Trash2, Edit2, Link as LinkIcon } from 'lucide-react';
import { MotivationalEarningSectionProps } from '../types';

export function MotivationalEarningSection({ motivationalEarning, currency, onUpdate }: MotivationalEarningSectionProps) {
  const [isEditingGoal, setIsEditingGoal] = useState(!motivationalEarning);
  const [desiredLifestyle, setDesiredLifestyle] = useState(motivationalEarning?.desiredLifestyle || '');
  const [requiredIncome, setRequiredIncome] = useState(motivationalEarning?.requiredIncome?.toString() || '');
      ...
      <div className="bg-royal/5 p-6 border-b border-royal/10">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center">
            ...
            <h3 className="text-lg font-bold text-slate-800">Motivational Earning</h3>
          </div>
        ...
        <p className="text-sm text-slate-500">
          Define your dream lifestyle and create a concrete plan to achieve the income required.
        </p>
      </div>
        ...
        {isEditingGoal ? (
          <form onSubmit={handleSaveGoal} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-medium">{currency === 'USD' ? '$' : currency}</span>
                <input
                  type="number"
          ...
          <div className="space-y-6">
            <div className="bg-royal/5 p-5 rounded-2xl border border-royal/10">
              ...
              <p className="text-sm text-slate-600 italic mb-4">"{motivationalEarning.desiredLifestyle}"</p>
              <div className="flex items-center justify-between border-t border-royal/10 pt-4">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Target Monthly Income</span>
                <span className="text-xl font-bold text-royal">{formatCurrency(motivationalEarning.requiredIncome, currency)}</span>
              </div>
            </div>
            ...
            <div>
              ...
              {isAddingAction && (
                <form onSubmit={handleAddAction} className="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-4 space-y-3">
                  <input
                    type="text"
                    value={actionTitle}
              ...
              {motivationalEarning.actions.length === 0 && !isAddingAction ? (
                <div className="text-center py-6 bg-slate-50 rounded-2xl border border-slate-100 border-dashed">
                  <p className="text-sm text-slate-500 mb-2">No actions planned yet.</p>
                  <p className="text-xs text-slate-400">Add steps like acquiring new skills, negotiating a raise, or starting a side hustle.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {motivationalEarning.actions.map(action => (
                    <div key={action.id} className={`p-4 rounded-2xl border transition-all ${action.completed ? 'bg-slate-50 border-slate-200 opacity-75' : 'bg-white border-royal/20 shadow-sm'}`}>
                      <div className="flex items-start">
                        <button 
                          onClick={() => toggleActionComplete(action.id)}
                          className="mt-0.5 mr-3 flex-shrink-0 text-royal hover:text-royal-dark transition-colors"
                        >
                          {action.completed ? <CheckCircle2 className="w-5 h-5" /> : <Circle className="w-5 h-5" />}
                        </button>
                        <div className="flex-1 min-w-0">
                          <h5 className={`font-bold text-sm ${action.completed ? 'text-slate-500 line-through' : 'text-slate-800'}`}>
                            {action.title}
                          </h5>
                          <p className="text-xs text-slate-500 mt-1 mb-2">{action.description}</p>
                          ...
                          {action.evidenceUrl && (
                            ...
                            >
                              <LinkIcon className="w-3 h-3 mr-1" /> View Evidence
                            </a>
                          )}
                        </div>
                        <button 
