...
import { Settings, Database, ChevronRight, ArrowLeft, Wallet as WalletIcon, User, RefreshCw } from 'lucide-react';
...
export function SettingsView({ state, onAddCategory, onUpdateCategory, onDeleteCategory, onSetCurrency, onUpdateRecurringTransaction, onDeleteRecurringTransaction, onImport, onClear }: SettingsViewProps) {
  const [activePage, setActivePage] = useState<'main' | 'categories' | 'account' | 'data' | 'recurring'>('main');

  if (activePage === 'categories') {
    return (
      <div className="space-y-4">
        <button onClick={() => setActivePage('main')} className="flex items-center text-slate-500 hover:text-slate-800 transition-colors mb-4">
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex items-center">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-slate-500">Change currency and manage account</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-slate-500">Add and edit transaction categories</p>
            </div>
          </div>
          <ChevronRight className="w-5 h-5 text-slate-400" />
        </button>
        ...
        >
          <div className="flex items-center">
            ...
            <div className="text-left">
              <h3 className="font-medium text-slate-800">Recurring Transactions</h3>
              <p className="text-xs text-slate-500">Manage your automated transactions</p>
            </div>
          </div>
          <ChevronRight className="w-5 h-5 text-slate-400" />
        </button>
        ...
        >
          <div className="flex items-center">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-slate-500">Import and export your data as CSV</p>
            </div>
          </div>
          <ChevronRight className="w-5 h-5 text-slate-400" />
        </button>
