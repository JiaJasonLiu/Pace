...
export function DataSettings({ state, onImport }: DataSettingsProps) {
	...
	const handleExportCSV = () => {
		...
		link.setAttribute(
			"download",
			`royal_budget_export_${new Date().toISOString().split("T")[0]}.csv`,
		);
	...
	const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		Papa.parse(file, {
			...
			complete: (results) => {
				try {
					const importedTransactions: Transaction[] = results.data.map(
						(row: any) => {
					...
					alert(
						"Failed to import CSV. Please ensure it matches the export format.",
					);
				}
		...
		<div className="space-y-6">
			<div className="bg-slate-800 text-white p-6 rounded-2xl shadow-lg">
				...
				<p className="text-slate-300 text-sm leading-relaxed">
					Export your data to analyze it in Excel or Google Sheets, or import
