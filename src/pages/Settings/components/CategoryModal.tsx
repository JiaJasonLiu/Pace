...
const AVAILABLE_ICONS = [
  'Utensils', 'Car', 'Home', 'Film', 'ShoppingBag', 'Zap', 'HeartPulse', 
  'Briefcase', 'Laptop', 'TrendingUp', 'Gift', 'MoreHorizontal', 'Coffee',
  'Plane', 'Smartphone', 'Wifi', 'Book', 'Music', 'Camera', 'Smile'
];

export function CategoryModal({ isOpen, onClose, onSave, onDelete, editingCategory, categories }: CategoryModalProps) {
  const [name, setName] = useState('');
  const [type, setType] = useState<TransactionType>('expense');
  const [icon, setIcon] = useState('MoreHorizontal');
  const [lifestyleType, setLifestyleType] = useState<'need' | 'want' | 'savings' | 'income' | 'none'>('need');
  ...
  useEffect(() => {
    if (editingCategory) {
      ...
      setLifestyleType(editingCategory.lifestyleType || (editingCategory.type === 'income' ? 'income' : 'need'));
    ...
    >
      <form id="category-form" onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-2">
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider">Lifestyle Bucket</label>
          <div className="flex rounded-lg overflow-hidden border border-slate-200 p-1 bg-slate-50">
            {type === 'expense' ? (
          ...
          {!mainCategoryId && (
            <p className="text-[10px] text-slate-400 italic">
              {type === 'expense' ? (
                <>
                  {lifestyleType === 'need' && "Essential expenses (Housing, Food, Bills)"}
                  {lifestyleType === 'want' && "Discretionary spending (Dining out, Hobbies)"}
                  {lifestyleType === 'savings' && "Money set aside for future goals"}
                </>
              ) : (
                <>
                  {lifestyleType === 'income' && "Regular income (Salary, Freelance)"}
                  {lifestyleType === 'savings' && "Income that goes directly to savings (Interest)"}
                  {lifestyleType === 'none' && "Income not tracked in lifestyle blueprint"}
                </>
              )}
            </p>
          )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Main Category</label>
            <select
              ...
              onChange={(e) => {
                ...
                if (newMainId) {
                  ...
                  if (mainCat && mainCat.lifestyleType) {
                    setLifestyleType(mainCat.lifestyleType);
                  }
                }
              }}
              disabled={editingCategory ? (categories || []).some(c => c.mainCategoryId === editingCategory.id) : false}
            ...
            >
              ...
              {(categories || [])
                .filter(c => !c.mainCategoryId && c.id !== editingCategory?.id && c.type === type)
                .map(c => (
            ...
            {editingCategory && (categories || []).some(c => c.mainCategoryId === editingCategory.id) && (
              <p className="text-[10px] text-rose-500 mt-1">Cannot be a subcategory because it has subcategories.</p>
            )}
          </div>
        </div>
