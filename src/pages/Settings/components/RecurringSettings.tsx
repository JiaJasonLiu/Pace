...
import type { RecurringSettingsProps } from "../types";
	...
	return (
		<div className="space-y-6">
					...
					{sortedRecurring.length === 0 ? (
						<div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
							...
							<p className="text-xs text-slate-400 mt-1">
								Create one from the Spending view.
							</p>
						</div>
					) : (
								...
								>
									...
									<div className="flex justify-between items-center">
										<div className="flex items-center gap-3">
											<div
												className={`w-10 h-10 rounded-full flex items-center justify-center ${r.type === "income" ? "bg-notion-green-light text-notion-green" : "bg-rose-50 text-rose-600"}`}
											>
										...
										<span
											className={`font-mono font-bold text-lg ${r.type === "income" ? "text-notion-green" : "text-slate-800"}`}
										>
											{r.type === "income" ? "+" : ""}
			...
			<TransactionModal
				...
				onSave={handleSave}
				onDelete={
					editingRecurring && editingRecurring.description !== "Salary"
						? () => {
								onDelete(editingRecurring.id);
								setIsModalOpen(false);
							}
