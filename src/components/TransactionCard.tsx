...
export const TransactionCard: React.FC<TransactionCardProps> = (props) => {
  ...
  const [bg, text] = getCategoryColors(category?.lifestyleType, transaction.type).split(' ');
  ...
  return (
    ...
    >
      <div className="flex items-center">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${finalBgColor} ${finalTextColor} ${isScheduled ? 'opacity-50' : ''}`}>
          {icon}
        </div>
        <div>
          <div className="flex items-center gap-2">
            {isScheduled && <Calendar className="w-3.5 h-3.5 text-amber-500" />}
            <p className={`font-medium ${isScheduled ? 'text-slate-400' : 'text-slate-800'}`}>{transaction.description || category?.name}</p>
            {isFixedCost && (
              <span className="bg-slate-100 text-slate-500 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider">Fixed</span>
            )}
          </div>
              ...
              <>
                {category && (
                  <span className="bg-slate-100 px-1.5 py-0.5 rounded text-[10px] mr-2 flex items-center">
                    ...
                    {mainCategory ? `${mainCategory.name} • ${category.name}` : category.name}
                  </span>
                )}
              </>
            )}
      ...
      <div className="flex items-center gap-3">
        <span className={`font-mono font-medium ${transaction.type === 'income' ? 'text-notion-green' : 'text-slate-800'} ${isScheduled ? 'line-through opacity-50' : ''}`}>
